...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen relative flex flex-row flex-1 h-full w-full bg-gray-50">
      ...
      <div
        className={cn(
          'hidden lg:block sticky start-0 z-[50] bg-white border-gray-200 shadow-sm',
          !isMobile
            ? 'hidden lg:block sticky start-0 z-[50] bg-white border-gray-200 shadow-sm'
            : 'fixed inset-y-0 start-0 z-[210] bg-white shadow-xl transform transition-transform duration-300 ease-in-out',
        )}
        style={!isMobile ? { width: sidebarWidth, top: 0, bottom: 0, blockSize: '100dvh',} :
         {
