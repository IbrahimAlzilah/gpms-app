...
const Sidebar: React.FC<SidebarProps> = ({ onClose, collapsed = false, onToggle }) => {
  ...
  const renderItem = (item: { name: string; href: string; icon: any }) => {
    ...
    return (
      ...
      >
        <Icon size={20} className="mr-3 rtl:ml-3 rtl:mr-0" />
  ...
  return (
    <aside
      className="h-full bg-white border-gray-200 overflow-y-auto shadow-sm flex flex-col"
      style={{ width: collapsed ? COLLAPSED_WIDTH : 256 }}
    >
